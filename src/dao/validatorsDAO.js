...
export default class ValidatorsDAO {
  ...
  static async getGetGeneralData(network) {
    try {
      let maxActiveStakePipeline = [
        {
          $group: {
            _id: null,
            maxActiveStake: { $max: "$active_stake" },
          },
        },
      ];
      ...
      let maxActiveStake = await validatorsDB
        .collection(`validators_general_${network}`)
        .aggregate(maxActiveStakePipeline)
        .toArray();

      maxActiveStake = maxActiveStake[0].maxActiveStake;
